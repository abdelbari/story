...
M1 and M2's device-independent work are complete. M2 remainder: print-pipeline and PDF-renderer polish after device testing. M3 OCR: first slice landed (Tesseract 5 via Tesseract4Android, ara+eng fast models in :pdf assets; remaining: page-segmentation and accuracy tuning on device, per-scan language picker, progress UI). M4: Review Mode UI on top of the FidelityReport data layer; Google Docs sync is gated on a product decision (the app ships with no INTERNET permission — adding it, even isolated to a sync flow, is the user's call).
