...
- **A marking says where; a comment says why, and the why was dropped:**
  ...
  each led back to the words it came from. Markdown has no comments and is
  given none: closing its round trip took long enough that adding text to
  it which cannot be read back is not worth the remarks. Nor does the PDF
  the phone exports carry them, which is what Word itself does when it
  prints a document without its markup.
