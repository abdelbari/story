...
- **App:** three conversion paths, all fully on-device — text/Markdown → Word (.docx), Word (.docx) → Markdown, and **PDF → Word** (text PDFs via the position-aware layout heuristics; scanned PDFs get an honest "OCR arrives with M3" message), plus **text/Markdown/Word → PDF** through the system print pipeline (engine-generated print-ready HTML rendered by WebView — Blink-quality RTL shaping — with "Save as PDF" in the print sheet) — via SAF pick-and-save, the share sheet (send a document to Morpho from any app), and "Open with" on supported types. UI localized in English, Arabic, French, Spanish, German with full RTL support, per-app language config, Material 3 dynamic color, and **no INTERNET permission in the manifest** — the Zero-Upload guarantee starts on day one.
...
M1 remainder: tagged-path figures. M2 remainder: print-pipeline polish after device testing, direct-to-file PDF export, full BiDi run analysis (UAX #9). M3: OCR (ML Kit + Tesseract). M4: Google Docs sync + Review Mode.
