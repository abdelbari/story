...
- **Known limitations (tracked):** a conversion survives the death of the process now — the last conversion, as it stands after the reader's corrections, is kept as text in the app's own files (`Autosave`) after every conversion and every correction, forgotten once saved or once another document is picked, and put back on the next launch converted and waiting to be saved; the empty stub a dialog created behind a dead process is still deleted. The reader locates the main part at the fixed OPC path `word/document.xml` rather than following the officeDocument relationship.
...
- **The edit screen is the editor now, and script runs in its WebView — in that one file and nowhere else.** Asked for on 5 September: the edit screen looked like a scroll of blocks with buttons, and was one (Review Mode); what was wanted is what Word and Docs are — the document itself, editable, with the type tools above the keyboard and the features every competitor has. The screen (`EditorScreen`) is the engine's editor page in a WebView with a toolbar over it: bold, italic, underline and strike; text colour and highlight from a palette; size; font; the styles from Title to Heading 3; alignment and justification; bullets and numbers with indent and outdent; links; notes; tables, with a second row of tools when the caret stands in a cell (rows and columns in and out, merge and split, shading, rules, a head row); pictures from the device, read and brought down to a document's size before they go in; find and replace across the document; word count; undo and redo; superscript, subscript and writing direction; and a button that walks the blocks the conversion was unsure of. Every tool calls the page's own API, so a button and a keystroke take the one path there is — page, bridge, engine, reply painted back — and the engine's tests hold every edit whichever made it. Closing the editor makes what was edited the conversion: the preview, the Fidelity Report and the save all follow it, and every edit the engine takes is kept against the process's death as before. Review Mode's code stays in the tree, reachable from nothing; what it did, the editor does in place. The plan's *Price one* — script in a WebView — is taken by this ask, since a page that can be typed in cannot exist without it, and taken exactly as the plan priced it: the app still declares no permission, so script has no network to reach; the editor's WebView is its own, with file access, content access, DOM storage, new windows, address following and every resource fetch refused, behind a page whose own policy allows no source at all, and the preview's WebView is untouched; there is one bridge object (`EditorBridge`) with three narrowly typed methods, each reading what the page sends as if a hostile document wrote it, and the page's script is held by a test to ask for those three and nothing else; and the CI guard was changed rather than skipped — it allows script and one `addJavascriptInterface` in exactly `EditorScreen.kt`, requires the five refusals to be written there, and still refuses file access in every WebView in the app. The screen's own look and feel on a phone is the one thing this machine cannot see; the Android build runs in CI alone.
...
Known and deliberate for 1.0: a PDF the phone exports carries neither
...
**The editor.** Asked for on 4 September — the editing side of the app
to work the way Docs and Word do, not as a list of blocks with buttons —
planned in `EDITOR-PLAN.md`, and built: the engine's whole edit algebra
and wire grammar, the page, and since 5 September the screen itself
(`EditorScreen`), which is the page in one locked-down WebView with the
tools of a word processor above the keyboard. That WebView runs script,
which is the plan's *Price one*, taken by the ask for a real editor (a
page that can be typed in is script or nothing) and taken exactly as the
plan priced it; the decisions log has the whole of it. What is left needs
a device, which neither CI nor this machine has: an input method
composing Arabic, touch selection handles, the keyboard showing and
hiding, and the true round trip through the bridge on a phone.
