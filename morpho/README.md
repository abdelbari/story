...
- **A table a scan carries is measured, diagnosed and not shipped:** with real recognition running here — the app's own models, at the app's own resolution and segmentation — a scanned table comes back as loose paragraphs, and every part of the reason is now known. Recognition finds the grid: on the fixture whose table is drawn, it reports four `ocr_separator` blocks down the page with their whole boxes and six across it recorded by their height alone, which are exactly the table's rules once the ones across are given the reach of the ones down. It finds the cells: each is its own line, correctly placed. Fed those as rules with the lines in the order they sit, **the ruled reader finds the table** — 1 region, the right one. Two things then stop it. The rule a scan draws is 3.2 points of ink where the reader admits 3, so four of the six are refused as too thick to be rules; and recognition cuts a table into text *columns*, so the reading orders its lines column by column, which breaks the ruled reader's rule that a table's lines are a contiguous run. Putting the lines back into rows by their baselines was tried and is worse, not better: it recovers no table (a centred head row aligns with nothing) and it destroys a real two-column page, turning its prose into a bogus eight-row table and losing 134 of 150 words. What is left is to stop the flow ordering from reordering lines inside a grid — a change to the ordering every PDF also goes through, for a gain measured on one fixture, which is not a trade to make on one fixture. Recorded here with the numbers so it is not re-derived.
