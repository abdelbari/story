...
and planned in `EDITOR-PLAN.md`. Everything of that plan a machine can do
is done, in the engine and in the page (the whole edit algebra, the wire
grammar, tables to the merged cell, find and replace, rich paste, notes,
the Fidelity Report as a layer), driven in headless Chromium against the
real engine after every action. What is not done needs either a device
or the owner's decision on script in the editor's WebView, which is the
plan's *Price one*: nothing of the editor is wired into the app until
that is decided, and the page's script is held by a test to ask the app
for exactly three things and reach for nothing else.
