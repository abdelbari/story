...
- **A table a scan carries comes back a table now, and the note that said it could not is corrected here:** the earlier reading of this was measured on one fixture — a table alone on a page — and concluded that the blocker was the column flow ordering, which cuts such a page into text columns and so scatters the table's lines out of the contiguous run the ruled reader needs. Three more pages were made and read with the app's own models at the app's own resolution and segmentation, and the conclusion does not survive them. On a page carrying prose *and* a ruled table — which is what an institution's report or form actually looks like — the flow ordering finds one column and **leaves the page alone**: the table's rows come back as rows, in page order, contiguous. The blocker there was something simpler and earlier that the one-fixture reading had hidden: **nothing read recognition's rules at all.** Tesseract looks for them and reports them as `ocr_separator` blocks, and no line of this engine had ever matched that word, so the ruled reader was handed a scanned table with no rules whatever and had nothing to work with. They are read now. They arrive lopsided — the ones down the page whole, the ones across it as `bbox 0 y 0 y'`, a position and no width at all — and that is taken as the signature rather than as damage: a rule with no width is one across the page, and the only reach it could have had is the reach of the ones down, so it is given that; one that arrives with a width of its own is left exactly as it is, so this does nothing at all if recognition ever starts reporting them whole. Measured over four real pages: the prose-and-table page comes back with its table and all four rows, the heading and the prose either side still prose; the two column pages carry no rules, so recognition reports no separators and their reading is byte-for-byte the reading they always had — which is the whole cost side of the trade, and why this needed no change to the flow ordering that every PDF also goes through. One limit stands, and chasing it down corrected the note a second time. A table alone on a page — no prose anywhere, which is page two of a long table or a form that fills the sheet — is still lost, and **not for either reason previously recorded**. Its rules are read and given their reach like any other; thinning them past the reader's three points of ink changes nothing; leaving the lines in the order they were painted changes nothing. The coordinates say why: recognition emits that page's cells **column by column in its own hOCR** — `Share` at y=118, then 80% at 147, 20% at 175, 64% at 204, 36% at 233, which is the rightmost column read top to bottom, and then the next column, and then the next. The lines arrive scattered, before any ordering of ours has run. So no change to the column flow ordering could recover it, which closes off the line of work the note pointed at: Tesseract's own block analysis reads a page that is nothing but a table as a page of text columns, and the same page with prose above and below it reads as rows. What would recover it is sorting the cells back into rows by their boxes — which is the baseline merging already measured here as worse, destroying a real two-column page and losing 134 of its 150 words. Recorded with the coordinates so the next attempt starts from the page and not from the pipeline.
