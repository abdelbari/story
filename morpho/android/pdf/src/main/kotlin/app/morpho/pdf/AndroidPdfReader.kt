...
class AndroidPdfReader(context: Context) {
    ...
    private fun extractFrom(
        ...
        run {
            ...
            val images = attempt { AndroidImageCapture().capture(doc, reading) } ?: emptyList()
            ...
            val fromTags =
                if (tagged) {
                    attempt { AndroidStructureTreeReader.read(doc, images, reading) }
                } else {
                    null
                }
    ...
    private fun drawn(document: PDDocument, figures: List<PdfDrawing>): List<PdfImage> =
        figures.take(MOST_FIGURES).mapNotNull { figure ->
            val cropped = AndroidPageImages.crop(
