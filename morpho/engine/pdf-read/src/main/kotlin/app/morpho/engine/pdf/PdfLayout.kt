...
/**
 * Untagged-PDF layout heuristics: clusters captured [PdfLine]s into
 * [Paragraph] blocks and promotes oversized short lines to headings.
 *
 ...
 * block indent yet.
 *
 * Headings: the body size is the median font size over all lines; a cluster
 * of at most [MAX_HEADING_LINES] lines totalling at most
...
internal object PdfLayout {
    ...
    fun reconstruct(lines: List<PdfLine>, confidence: Float): DocumentModel {
        val clusters = cluster(lines)
        val bodySize = median(lines.map { it.maxFontSize })
        val kindBySize = headingKinds(clusters, bodySize)
        val blocks = clusters.map { clusterLines ->
            val kind =
        ...
        val rtlCount = blocks.count { it.style.direction == TextDirection.RTL }
