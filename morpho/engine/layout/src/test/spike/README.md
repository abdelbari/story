...
`EditorLookTest` writes an Arabic paper and an English one, with everything
the page draws, under `layout/build/editor-look/`; then, from this directory,

    node editor-look.mjs

photographs each in headless Chromium at a phone's size and a tablet's, in
light and in dark, and once more with words selected and a picture picked,
into `look/`, which is not committed. The photographs are for a person to
look at; nothing asserts on them.
