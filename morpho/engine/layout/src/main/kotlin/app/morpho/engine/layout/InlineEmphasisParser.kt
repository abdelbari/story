 ...
 *   tilde is a tilde.
 * - `\*`, `\~`, `\\`, `\|`, `\[`, `\]`, `\#`, `\-`, `\.` and `\)` are the
 *   characters themselves — exactly the set [MarkdownWriter] escapes, so
 *   write→import round-trips; a backslash before any other character stays
 *   a literal backslash. Brackets are among them because a bracket that
 *   opens nothing must not be read as the link or the note's mark it
 *   resembles, and the last four because a paragraph that merely begins
 *   `#`, `-` or `1.` must not be read as the heading or the list item it
 *   is shaped like.
